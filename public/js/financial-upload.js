    ...
    return `
      <div class="document-item" style="padding: 12px; border-bottom: 1px solid var(--border-color);">
        ...
        <div style="display: flex; gap: 12px; margin-bottom: 8px;">
          ...
          <!-- 右側：操作按鈕；瀏覽＝開啟唯讀 Google Sheet 網頁（後端會導向或先轉檔再導向） -->
          <div style="display: flex; flex-direction: column; gap: 6px; min-width: 80px;">
            <a href="/api/financial/view-as-sheet/${doc.id}?userId=${encodeURIComponent(userId)}" target="_blank" rel="noopener" class="btn btn-sm btn-secondary" style="width: 100%; padding: 6px 12px; font-size: 13px; text-align: center; text-decoration: none; box-sizing: border-box;">👁️ 瀏覽</a>

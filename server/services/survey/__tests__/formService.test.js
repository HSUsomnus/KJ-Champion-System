...
describe('formService.submitForm', () => {
  ...
  test('新姓名（不在既有名單）→ 先寫入 survey_members(pending) 再寫入 submission', async () => {
    db.query
      ...
      .mockResolvedValueOnce({ rows: [] }) // insert new member for 'name'
      .mockResolvedValueOnce({ rows: [] }) // insert new member for 'recommender'
    ...
    // 查表單 + 查名單 + 2 次新姓名 insert + submission insert = 5

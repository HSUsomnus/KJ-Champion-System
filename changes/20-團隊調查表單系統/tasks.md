...
- [ ] **20.18** `[Codex CX-1]` 建 `routes/admin.js`：`GET /admin/forms`、`GET /admin/forms/:id/submissions`（過 `requireAdminSession`）；`server.js` 唯一註冊 `app.use('/admin',...)`
- [ ] **20.19** `[Codex CX-1]` jest：無 bearer→401、非管理者→403、管理者→200+筆數
...
- [ ] **M.1** `[使用者]` 認證：非管理者被擋、管理者放行、**撤權後 token 立即失效**、**F5/PWA reload/crash 後需重登（預期非 bug，M-2 覆核）**
- [ ] **M.2** `[使用者]` 前台送出：非法值/超量/超頻被擋（400/429）；正常送出寫入
- [ ] **M.3** `[使用者]` 未填名冊分組/進度與手算一致、pending 未進母數、同名重複不超 total（M-3 覆核）
- [ ] **M.4** `[使用者]` 匯出 CSV/xlsx 開檔確認（含公式中和）
- [ ] **M.5** `[使用者]` confirmed 名單經分享 token 仍全揭露 → 人眼確認可接受（M-1 覆核隱私驗收）

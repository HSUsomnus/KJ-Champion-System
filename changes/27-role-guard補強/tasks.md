...
> 純規則檔，依 workflow skill 測試表跳過 Unit Test，以 spec 的 G1–G4 gate 取代。
>
...
## Phase 3 — 上線（規則類直推 main；等 change 26 上線後才執行）

- [ ] 3.1 確認 change 26 已上線 main（`git log origin/main` 可見 change 26 commits），未上線則停下回報
- [ ] 3.2 更新 `.claude/now.md`「最近推送」段（與推送同 commit）
- [ ] 3.3 機密檢查（git status 無 .env / Key/ / 金鑰 *.json）
- [ ] 3.4 列推送清單，等使用者明確確認
- [ ] 3.5 push main（CCR/VPS 403 則交自包含指令給使用者）→ `bash scripts/sync-branches.sh`

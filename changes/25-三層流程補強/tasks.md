...
- [x] 0.2 建分支：`git fetch origin claude/project-spec-review-95epn4 && git checkout -b m_b_三層流程補強 origin/claude/project-spec-review-95epn4 && git push -u origin m_b_三層流程補強`

## Phase 1 — 角色 command 補強

- [x] 1.1 `規劃.md` 停止點交接訊息模板改為含 spec 分支 + 起手式三行（spec 3.1 原文替換）
- [x] 1.2 `實作.md` 加「spec 取得」段（spec 3.2）
- [x] 1.3 `實作.md` 收尾筆記四段改五段，插入「主 session 連結」（spec 3.3）
- [x] 1.4 `規劃.md` 停止點前加「確認閘」段——未獲確認前禁止 commit/push/開 PR（spec 3-1.4）
- [x] 1.5 `實作.md` 職責段改「每 Phase commit 後立即 push 遠端」（spec 3-1.5）
- [x] 1.6 跑 8.1 gate 全 ✅，commit（`chore:` + trailer，tasks.md 同批）
...
- [x] 4.1 CCR 403 條目句尾補「補做指令必須自包含」；新增簽章 N 屬正常條目（spec 六-1、2）
- [x] 4.2 change 12 行替換為封存記錄（spec 六-3；封存 tag 與刪分支本身是使用者自辦，見 spec 九-2）
- [x] 4.3 跑 8.4 gate 全 ✅（常駐 token 估算 3201 < 3500），commit

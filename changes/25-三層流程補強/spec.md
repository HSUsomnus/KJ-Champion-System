# Change 25 — 三層流程補強（change 24–25 實地驗證發現的五個缺口）
...
> 不自包含）；另有第五個缺口由 change 25 規劃過程本身暴露（顧問未經確認即 push spec）。
> 本 change 將五個缺口制度化修補，**零產品程式碼**，全部是 `.claude/` 規則文字。
...
## 三、Phase 1 — `/實作`、`/規劃` command 補強（缺口 1、3、5）

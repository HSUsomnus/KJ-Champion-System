...
重新評估「桌機體驗不佳」的解法時，考慮過三條路：

| 方案 | 做法 | 成本 | 結論 |
|---|---|---|---|
| **A. 窄欄置中** | `max-w-md` 置中 + 兩側留白，維持手機優先單欄 | 低 | ✅ **採用** |
| B. 響應式桌機版 | 桌機改多欄 / 側邊欄導航取代 FAB、寬版內容 | 高（需重設計 + 大量測試） | ❌ 不採用 |
| C. 維持現狀 | 不處理 | 0 | ❌ 拉滿可讀性差 |

**決定採用 A 的理由（使用情境前提）**：實際使用**以手機為主，桌機只是偶爾 / 維護時看**。在此前提下：

- 方案 B 投資報酬率低 — 為「偶爾用」的桌機重做整套響應式版面不划算
- 方案 A 以最低成本解決「拉滿難讀」的核心痛點，且**完全保留手機優先體驗**（限寬值 ≥ 手機寬，手機零回歸）
- FAB 維持不變（手機模式），只需在桌機對齊置中欄，不改成桌機式頂部導航

> 若未來桌機升級為主要使用情境，再另開 change 走方案 B，不在本 change 範圍內。

...
- **codex exec 委派時偶爾撞 PowerShell 擋 `npx.ps1`**：Codex 自己內部跑 jest 有時會被 Windows PowerShell 執行原則擋下，牠會自行改用 `npx.cmd jest` 或 `npx jest`（Bash）排除，屬已知小插曲、不影響最終結果，委派 prompt 不必特別處理，Codex 自己會繞過
...
- **本機桌機（win32 VSCode extension）已可直接調度 Codex CLI，不需經 VPS 指揮官架構**：全域裝了 `@openai/codex`（`npm install -g @openai/codex`，`codex-cli 0.144.6`），已用 ChatGPT 帳號登入（`codex login status` → `Logged in using ChatGPT`）。委派方式：`codex exec -s workspace-write "任務描述"`，在 repo root 執行（讓它讀到 `AGENTS.md`）。本機 npm/node/jest/vitest 全部可直接跑（不受 CCR 沙箱限制，因為這不是 CCR 環境）。change 20 的 Codex 委派套件（PUB-B/C、CX-1~4）全部靠這條路徑完成，`.claude/指揮官架構.md` 記的 VPS 版本這次沒用到，是備援路徑不是必經路徑。委派時記得先問要不要在本機直接調度，不要預設一定要走 VPS。
